# Part 4-Automated Decryption (question1-3)
import os
import random
...
textMod = ""
while True:
    modType = input("The cipher mode('encrypt' or 'decrypt' or 'auto-decrypt'):")
    if modType=="encrypt" or modType=="decrypt" or modType=="auto-decrypt" : break
    print("Please enter 'encrypt' or 'decrypt' or 'auto-decrypt'!")

if modType != "auto-decrypt":
    while True:
...
if modType=="encrypt":
    ...
    print(data)
    printAns(ans)
elif modType=="auto-decrypt":
    dictionaries =[]
    err = os.path.exists("./words.txt")
    if err:
        f = open("./words.txt")
        dictionarie = f.read() 
        f.close()
        dictionaries = dictionarie.split()
    else :
    ...
    for i in range(26):
        flag = False
        tempData = decode(textTemp[0],i)
        tempWord = analyze(tempData)
        for j in tempWord["words"]:
            if j[0] in dictionaries:
                flag = True
                break
        if flag:
            print("Decrypted msessage(the first line):")
            print(tempData)
            yn =""
            while True:
                yn = input("Has the line been successfully decrypted?('yes' or 'no'):")
                if yn=="yes" or yn=="no":break
